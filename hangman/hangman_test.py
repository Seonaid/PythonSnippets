...
#additional unit tests
    def test_only_single_character_guesses_are_allowed(self):
        game = Hangman('foobar')
        with self.assertRaisesWithMessage(ValueError):
            game.guess('ab')

    def test_character_must_be_letter(self):
        game = Hangman('foobar')
        with self.assertRaisesWithMessage(ValueError):
            game.guess('1')

    def test_guess_is_not_case_sensitive(self):
         game = Hangman('foobar')
         game.guess('F')
         self.assertEqual(game.remaining_guesses, 9)
         self.assertEqual(game.get_masked_word(),'f_____')
